...
//NOTE: end tag before a start tag assumes that the tag value has not changed
// ... if it has then the editor needs to do something different.

//Cursor. Overlapping regions should be merged before actions are performed.
//Normalise cursor function
...
function level(obj) {
	var indent = obj.indent || 0;
	if (obj._type)
		return indent + (levels.hasOwnProperty(obj._type) ? levels[obj._type] : 1);
	if (obj.type)
		return indent + (levels.hasOwnProperty(obj.type) ? levels[obj.type] : 1);
...
function prefix(doc, start, end, expandAll, _lvl, _lin) {
	...
	if (end >= offset && start <= 0)
		lin.push({_type:doc.type, id:doc.id, attr:doc.attr, _tags:doc._tags, indent:doc.indent});
	var cl;
	var s;
